...
#pathToCountriesJson='/home/user/vscode/yulia/aps/patterns/countries.json'
...
#абстрактная фабрика для создания БД с доп логикой создания
...
class FakeDB(IFakeDB):
    def __init__(self,portNumber : int, pathToListCountries:string, countOfTickets : int ): 
        ...
        self.__mockUpBase(pathToListCountries, countOfTickets)
    ...
    def __mockUpBase(self, pathToListCountries:string, countOfTickets : int):        
        for currentTicketNumber in range(0, countOfTickets):
            randomTicket = self.__generateRandomTicket(pathToListCountries)
            self.__ListOfTickets.append(randomTicket)
    
    def __generateRandomTicket(self, pathToListCountries:string ) -> dict:
        ...
        readedJson=[]
        with open(pathToListCountries, encoding='utf-8') as f:
        ...
        randomTicket = { "ticketNumber" : randomTcketNumber, "flightNumber" : randomFlightNumber, "departureTime" : randomDepartureDate.strftime('%d.%m.%Y %H:%M'), "arriveTime" : randomArriveDate.strftime('%d.%m.%Y %H:%M'), "bagage" : randomBagage, "arrivedCountry" : randomArriveCountry, "departureCountry" : randomDepartureCountry, "arrivedCity" : randomArriveCity, "departureCity" : randomDepartureCity }
...
#поскольку приложение не общается с БД нарпямую - создаем спецальный драйвер для общения с БД - он же будет заниматься
#приведением голых данных к виду использующимуся в приложении - в том числе и адаптацией весов при создании обьекта Билет 
            
#также используем синглтон, он в зачет не идет, но по логике как будто направшивается - драйвер на приложение на один тип должен быть один
class FakeDBDriver:
    __instance = None
    ...
    def readDataFromDB(self) -> list:
        ...
        print(nonKgCountryList)  
        
        for ticket in fakeDataTicketList:
            
            ticketNumber=ticket['ticketNumber']
            flightNumber=ticket['ticketNumber']
            ...
            newTicket=TicketModule.Ticket(ticketNumber,flightNumber,departureTime,arriveTime,ticketBagage,arrivedCountry,departureCountry,departureCity,arrivedCity)
